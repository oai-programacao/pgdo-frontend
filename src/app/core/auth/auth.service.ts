...
// AQUI É CODIGO DA IA PARA CIMA DO PEDRÃO
...
export class AuthService {
  ...
  private isRefreshingToken = false;
  private tokenRefreshedSubject = new BehaviorSubject<boolean | null>(null);
  ...
  constructor() {
    ...
    if (initialUser) {
      this.scheduleProactiveRefresh(this.getAccessToken()!);
      // Reconnect SSE if page reload with valid token
  ...
  private getUserFromToken(): AuthenticatedUser | null {
    ...
    try {
      ...
      if (decodedToken.exp && decodedToken.exp * 1000 < Date.now()) {
        console.warn("Token expired at init.");
        this.clearTokens();
        return null;
      }
    ...
    return this.http
      ...
      .pipe(
        tap((response) => {
          this.storeTokensAndScheduleRefresh(response);
          this.currentUserSubject.next(this.getUserFromToken());
          this.sseService.connect(); // Start SSE after login
        }),
  ...
  refreshToken(): Observable<LoginResponseDto> {
    ...
    return this.http
      .post<LoginResponseDto>(`${this.apiUrl}/refresh`, { refreshToken: currentRefreshToken })
      .pipe(
        tap(response => {
          this.storeTokensAndScheduleRefresh(response);
          this.currentUserSubject.next(this.getUserFromToken());
          ...
          this.sseService.connect(); // Reconnect SSE after refresh
        }),
        catchError(err => {
          this.isRefreshingToken = false;
    ...
    try {
      ...
      if (proactiveRefreshDelay > 0) {
        this.tokenExpirationTimer = setTimeout(() => {
          ...
          this.refreshToken().subscribe({
            next: () => console.log("Token refresh OK."),
            error: err => console.error("Token refresh failed:", err),
          });
        }, proactiveRefreshDelay);
      }
    } catch (error) {
      console.error("Failed to decode token for scheduling:", error);
    }
  }
  ...
  private createTokenResponseFromStorage(): Observable<LoginResponseDto> {
    ...
    return throwError(() => new Error("No tokens in storage."));
  }
