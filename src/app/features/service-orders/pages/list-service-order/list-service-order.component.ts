...
export class ListServiceOrderComponent implements OnInit, OnDestroy {
  ...
  statusOptions: ServiceOrderStatus[];
  serviceOrderTypeOptions: TypeOfOs[];
  cityOptions: City[];
  ...
  private loadServiceOrdersPolling(): void {
    ...
    this.serviceOrderService.findAll(filters, page, this.rows).subscribe({
      next: (dataPage: CustomPageResponse<ViewServiceOrderDto>) => {
        ...
        if (
          JSON.stringify(this.lastServiceOrdersSnapshot) !==
          JSON.stringify(dataPage.content)
        ) {
    ...
    this.route.queryParamMap
      ...
      .subscribe((params) => {
        ...
        const filtersToSet: any = {
          contractNumber: params.get("contractNumber")
            ? Number(params.get("contractNumber"))
            : null,
          ...
          startDate: params.get("startDate")
            ? new Date(params.get("startDate") + "T00:00:00")
            : null,
          endDate: params.get("endDate")
            ? new Date(params.get("endDate") + "T23:59:59")
            : null,
  ...
  private formatFilters(rawFilters: any): ServiceOrderFilters {
    ...
    if (rawFilters.startDate && rawFilters.startDate instanceof Date) {
      formattedFilters.startDate = this.datePipe.transform(
    ...
    if (rawFilters.endDate && rawFilters.endDate instanceof Date) {
      formattedFilters.endDate = this.datePipe.transform(
        rawFilters.endDate,
        "yyyy-MM-dd"
      );
    } else {
  ...
  getStatusLabel(status: ServiceOrderStatus): any {
  ...
  getSubTypeOsLabel(
    subType: SubTypeServiceOrder | SubTypeServiceOrder[]
  ): string {
    if (Array.isArray(subType)) {
      return subType.map((s) => SubTypeServiceOrderLabels[s] || s).join(", ");
    }
    return SubTypeServiceOrderLabels[subType] || subType;
  }

  getTypeOfOsLabel(type: TypeOfOs | TypeOfOs[]): string {
    if (Array.isArray(type)) {
      return type.map((t) => TypeOfOsLabels[t] || t).join(", ");
    }
    return TypeOfOsLabels[type] || type;
  }
  ...
  getStatusSeverity(status: ServiceOrderStatus): any {
    const statusMap: Record<string, string> = {
      ...
      TARGETED: "info",
      CANCELED: "danger",
      TECHNICAL_INFEASIBILITY: "danger",
      RESCHEDULED: "warning",
      PARTIALLY_EXECUTED: "warning",
      RESCHEDULED_PENDING: "warning",
    };
