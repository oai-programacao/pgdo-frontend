...
export class OffersListComponent implements OnInit, OnDestroy{
  ...
  private subscribeToRealtimeUpdates(): void {
    this.sseSubscription = this.sseService.notificationEvents$.subscribe(notification => {
      console.log('Real-time notification received, reloading requested offers...', notification);
      
      // Aqui você poderia ser mais inteligente, ex: verificar se a notificação
      // é sobre uma nova oferta solicitada antes de recarregar.
      // Por agora, vamos recarregar sempre que uma notificação chegar.
  ...
  confirmDelete(): void {
    if (!this.offerToDelete || this.quantityToDelete <= 0) {
      this.messageService.add({ severity: 'warn', summary: 'Atenção', detail: 'Quantidade para deletar deve ser maior que zero.' });
      return;
    }
    ...
    this.adminOffersService.deleteByCriteria(
      ...
      next: () => {
        this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: `${this.quantityToDelete} oferta(s) deletada(s).` });
      ...
      error: (err) => {
        ...
        this.messageService.add({ severity: 'error', summary: 'Erro na Exclusão', detail });
      }
    });
  }
  ...
  acceptOffer(offerId: string): void {
    this.adminOffersService.acceptOffer(offerId).subscribe({
      next: (updatedOffer) => {
        this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Oferta aceita com sucesso!' });
        this.loadRequestedOffers(); // Recarrega a lista de ofertas solicitadas
      },
      error: (err) => {
        this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Falha ao aceitar oferta.' });
        console.error(err);
      }
    });
  }
  ...
  rejectOffer(offerId: string): void {
    this.adminOffersService.rejectOffer(offerId).subscribe({
      next: (updatedOffer) => {
        this.messageService.add({ severity: 'info', summary: 'Sucesso', detail: 'Oferta rejeitada com sucesso!' });
        this.loadRequestedOffers(); // Recarrega a lista de ofertas solicitadas
      },
      error: (err) => {
        this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Falha ao rejeitar oferta.' });
        console.error(err);
      }
    });
  }
  ...
  createManyOffers(): void {
    ...
    this.adminOffersService.createAvailableOffer(dto).subscribe({
      next: (createdOffers: any) => {
        this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: `${createdOffers.length} ofertas foram criadas!` });
        this.isSubmitting = false;
        this.displayCreateDialog = false;
        this.loadOffers(); // Recarrega a lista para mostrar as novas ofertas
      },
      error: (err) => {
        this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Falha ao criar ofertas.' });
        console.error(err);
        this.isSubmitting = false;
      }
    });
